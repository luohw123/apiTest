 ...
 * java先通过equals方法判断存储位置，如果不同直接存入；如果通过equals方法比较现在要存入的对象与结合中的某个对象相等，那么它就会再根据hashcode来判断它们是否hashcode也相等，如果相等那就存不进去了，说明它们确实是同一个对象，不等就可存入。所以一般在写程序的时候,两个对象你认为它们不同就去覆盖equals方法。这样可以提高效率，不要让JAVA再去判断hashcode
 */
/**
